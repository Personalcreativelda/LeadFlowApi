...
export function usePlanValidation(user: UserProfile | null): PlanValidation {
  return useMemo(() => {
    ...
    // Verificar limites E acesso premium
    const canAddLead = isPlanActive && (isUnlimitedLeads || remainingLeads > 0);
    const canSendMessage = isPlanActive && hasPremiumAccess && (isUnlimitedMessages || remainingMessages > 0);
    const canImportLeads = isPlanActive && hasPremiumAccess && (isUnlimitedLeads || remainingLeads > 0);
