...
import { Upload, FileSpreadsheet, Loader2, CheckCircle2, AlertCircle, Download, X, Star, Zap, Rocket } from 'lucide-react';
...
export default function ImportarLeadsModal({ isOpen, onClose, onSuccess, userPlan = 'free' }: ImportarLeadsModalProps) {
  ...
  const importLimits: Record<string, number> = {
    free: 50,          // Free: 50 leads
    business: 250,     // Business: 250 leads
    enterprise: 1000,  // Enterprise: 1000 leads
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Check if the number of leads exceeds the plan limit
      if (leads.length > maxImport) {
        throw new Error(`Você excedeu o limite de importação para o seu plano (${maxImport} leads)`);
      }
      ...
      // Send to backend
      const response = await apiRequest('/leads/import', 'POST', { leads });

      console.log('📡 Resposta da API:', response);

      if (response.success) {
        // Validar estrutura da resposta
        const imported = response.result?.imported || response.imported || leads.length;
        const skipped = response.result?.skipped || response.skipped || 0;
        ...
        throw new Error(response.error || 'Erro ao importar leads');
      }
    } catch (err: any) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="bg-white rounded-2xl max-w-xl w-full max-h-[85vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className={`relative p-4 rounded-xl border-2 transition-all ${
              ...
              <div className="mt-3">
                ...
                <p className="text-xs text-gray-600">leads por importação</p>
              </div>
            </div>
          ...
          {result && (
            <Alert className="border-green-200 bg-green-50">
              ...
              <AlertDescription className="text-green-900">
                <p className="font-medium mb-1">Importação concluída!</p>
                <p className="text-sm">
                  ✓ {result.imported} leads importados com sucesso
                  {result.skipped > 0 && ` • ⚠ ${result.skipped} ignorados (duplicados ou inválidos)`}
                </p>
              </AlertDescription>
            </Alert>
          )}
