...
export default function BackendStatusModal({ isOpen, onClose }: BackendStatusModalProps) {
  ...
  const checkBackendStatus = async () => {
    ...
    try {
      ...
      // Try to fetch leads
      const projectId = 'rfzmpkdtasgwkopiboya';
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-4be966ab/leads`,
        {
          method: 'GET',
          headers: {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-white rounded-2xl max-w-lg w-full shadow-2xl">
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="p-3 bg-gray-50 rounded-lg">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-600">Token de autenticação:</span>
          ...
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-xs text-blue-900">
              ...
              {status === 'offline' && <><br/>• Os dados podem estar sendo simulados (modo mock)</>}
              {status === 'offline' && <><br/>• Importações e exclusões NÃO estão funcionando</>}
            </p>
          </div>
